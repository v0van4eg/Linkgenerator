...
def generate_yandexmarket_xlsx(image_data, client_name):
    """
    Генерирует XLSX документ для ЯндексМаркет (базовый шаблон)
    """
    try:
        wb = Workbook()
        ws = wb.active
        ws.title = "YandexMarket Images"

        # Заголовки для ЯндексМаркет
        headers = ["Артикул", "Основное изображение"]
        for i in range(1, 11):
            headers.append(f"Дополнительное изображение {i}")
        ...
        # Создаем строки для каждого артикула
        for article, urls in articles.items():
            row_data = [article]  # Артикул

            # Добавляем ссылки на изображения
            if urls:
                row_data.append(urls[0])  # Основное изображение
